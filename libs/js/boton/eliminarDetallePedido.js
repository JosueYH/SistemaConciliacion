...
botones.forEach(boton=> {
  boton.addEventListener("click",function(){
    //console.log("Hola");
    const detalle=this.dataset.iddetallepedido;
    //console.log(detalle);
    const confirm=window.confirm("Estas Seguro de Querer Eliminar Este Producto?"+detalle);
    //respuesta
    if (confirm) {
      //solicitud AJAX
      httpRequest("http://localhost/coop/pedido/eliminado/"+detalle,function () {
...
cancelar.forEach(boton=> {
  boton.addEventListener("click",function(){
    //console.log(detalle);
    const confirm=window.confirm("Estas Seguro de Cancelar El Factura?");
    //respuesta
    if (confirm) {
      //solicitud AJAX
      httpRequest("http://localhost/coop/pedido/cancelado",function () {
        //console.log(this.responseText);//devolucion del contenido d ela solicitud
