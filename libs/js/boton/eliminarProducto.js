...
botones.forEach(boton=> {
  boton.addEventListener("click",function(){
    //console.log("Hola");
    const id_producto=this.dataset.idproducto;
    //console.log(id_producto);
    const confirm=window.confirm("Estas Seguro de Querer Eliminar Este Porducto?"+id_producto);
    //respuesta
    if (confirm) {
      //solicitud AJAX
      httpRequest("http://localhost/coop/consultarproducto/eliminarProducto/"+id_producto,function () {
        //console.log(this.responseText);//devolucion del contenido d ela solicitud

        //document.querySelector("#respuesta").innerHTML=this.responseText;
        const tbody= document.querySelector("#tbody-producto");//tbody id
        const fila = document.querySelector("#fila-"+id_producto);//tr id
